...
def admin_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if admin.role.auths != '':
            ...
            auths = list(map(lambda v: int(v),
                             filter(lambda v: v != '', auths.split(','))))
...
def movie_add():
    ...
    if form.validate_on_submit():
        ...
        if not os.path.exists(app.config['UP_DIR']):
            ...
            os.chmod(app.config['UP_DIR'], 664)
...
def preview_edit(id):
    ...
    if form.validate_on_submit():
        ...
        preview_count = Preview.query.filter_by(title=data['title']).count()
        if preview.name != data['title'] and preview_count == 1:
            flash('标签已经存在！', 'err')
...
def role_edit(id):
    ...
    if request.method == 'GET':
        form.auths.data = list(map(lambda v: int(v),
                                   filter(lambda v: v != '', role.auths.split(','))))
    if form.validate_on_submit():
        data = form.data
        role_count = Role.query.filter_by(name=data['name']).count()
        ...
        role.name = data['name']
        role.url = ','.join(map(lambda v: str(v), data['auths']))
        db.session.add(role)
        db.session.commit()
        flash('编辑角色成功！', 'ok')
        return redirect(url_for('admin.role_edit', id=id))
...
def auth_edit(id):
    ...
    if form.validate_on_submit():
        data = form.data
        auth_count = Role.query.filter_by(name=data['name']).count()
